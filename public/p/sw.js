  ...
  self.clients.matchAll().then((clients) => {
    clients.forEach((client) => {
      client.postMessage({
        ...
        url: url,
        name:
          wispConfig.servers.find((s) => s.url === url)?.name ||
          "Unknown Server",
        latency: latency,
      });
    });
  ...
  e.response = (async () => {
    ...
    for (let i = 0; i <= MAX_RETRIES; i++) {
      try {
        ...
        console.warn(
          `Scramjet retry ${i + 1}/${MAX_RETRIES} for ${e.url} due to: ${err.message}`,
        );
    ...
    if (
      ...
      if (
        currentHealth &&
        currentHealth.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES
      ) {
        ...
        for (const server of wispConfig.servers) {
          ...
          if (
            !serverH ||
            serverH.consecutiveFailures < MAX_CONSECUTIVE_FAILURES
          ) {
            // Ping to verify it's actually working
            const pingResult = await pingServer(server.url);
            if (pingResult.success) {
              console.log(
                `SW: Auto-switching to ${server.url} due to failures on current server`,
              );
    ...
    return new Response(
      "Scramjet Fetch Error: " + (lastErr?.message || String(lastErr)),
      { status: 502 },
    );
  })();
});
